...
<!--
    ...
    アプリ開発
    MVCアーキテクチャをベースにした読み込み制御、ログイン機能の導入
    例外処理、エラーメッセージ実装 F1

    オリジナルアプリ 山レコ開発
    目的　自分が登った山について一目でわかる物が欲しかったから
    進捗状況　　ログイン機能の実装は完了。ログイン画面におけるリダイレクト処理
    　　　　　　メッセージ出力機能の導入とエラーキャッチ。ログアウト機能
              bootstrapを使って見栄えの調整。見直し
              topic編集画面の作成.投稿とコメント機能の実装
    　ここまでの内容を１から見直し
    ...
    オリジナルアプリ　タスク管理
    目的　タスクを見える化。
    　　　ユーザーごとにタスクを管理。内容の追加、編集、削除機能を実装。３日で作成完了
